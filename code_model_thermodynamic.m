...
FRC=x(1); % Recompression fraction: variable
ksaiLTR=x(2); % Enthalpy efficiency of low temperature recuperator: variable
ksaiHTR=x(3); % Enthalpy efficiency of high temperature recuperator: variable
...
nP7star=1;
   ...
   if abs((mT8star-T8)/T8)>0.00001
    ...
    NuhHTR=0.040*(1.077*RehHTR)^(0.78)*PrhHTR^(0.4);
    %Nu and f of cold side
    fcHTR=0.47*(1.2/(Wchyi/2))^(-0.255)*(RecHTR)^((-0.173)*(1.2/(Wchyi/2))^(-0.274)); %冷侧f
    NucHTR=0.040*(1.077*RecHTR)^(0.78)*PrcHTR^(0.4);
   ...
   Exsunbujian=Exsunturbine+ExsunHTR+ExsunLTR+ExsunmainCom+ExsunreCom;% total
